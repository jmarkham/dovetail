...
public class Util {
	...
	public Util(DovetailConfiguration conf) {
		this.conf = conf;
	}

	public void addManagementUser(String jbossHome, String userId,
			String password) {

		String realm = conf
				.get(DovetailConfiguration.DOVETAIL_JBOSS_MANAGEMENT_REALM);
	...
	public void addDomainServerGroup(String jbossHome, String serverGroupName) {
		try {
			...
			// clear out any servers
			NodeList existingServerGroups = root
					.getElementsByTagName("server-groups");
			for (int i = 0; i < existingServerGroups.getLength(); i++) {
				Element svrGroups = (Element) existingServerGroups.item(i);
				svrGroups.getParentNode().removeChild(svrGroups);
			}
			...
			heap.setAttribute("max-size",
					conf.get(DovetailConfiguration.DOVETAIL_CONTAINER_MEMORY)
							+ "m");
	...
	public void addDomainServer(String jbossHome, String serverGroupName,
			String serverName, int portOffset) {
		try {
			String fileName = String.format(
			...
			// clear out any servers
			NodeList existingServers = root.getElementsByTagName("servers");
			for (int i = 0; i < existingServers.getLength(); i++) {
				Element svr = (Element) existingServers.item(i);
				svr.getParentNode().removeChild(svr);
			}
	...
	public void addDomainController(String jbossHome, String controller,
		...
		try {
			...
			addManagementUser(
					jbossHome,
					"master",
					conf.get(DovetailConfiguration.DOVETAIL_JBOSS_DOMAIN_MASTER_PASSWORD));
			addManagementUser(
					jbossHome,
					"slave",
					conf.get(DovetailConfiguration.DOVETAIL_JBOSS_DOMAIN_SLAVE_PASSWORD));
			...
			// clear out any domain controllers
			NodeList domainControllers = root
					.getElementsByTagName("domain-controller");
			for (int i = 0; i < domainControllers.getLength(); i++) {
				Element dc = (Element) domainControllers.item(i);
				dc.getParentNode().removeChild(dc);
			}
			...
			if (controller.equals(host)) {
				...
				remote.setAttribute(
						"security-realm",
						conf.get(DovetailConfiguration.DOVETAIL_JBOSS_MANAGEMENT_REALM));
				...
				for (int i = 0; i < securityRealms.getLength(); i++) {
					...
					if (securityRealm
						...
						String value = new String(
								Base64.encodeBase64(conf
										.get(DovetailConfiguration.DOVETAIL_JBOSS_DOMAIN_SLAVE_PASSWORD)
										.getBytes()));
